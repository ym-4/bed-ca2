document.addEventListener("DOMContentLoaded", function () {
        ...
        <div class="col-md-6 mb-4">
          <div class="card h-100">
            ...
            <div class="card-body">
              ...
              <button class="btn btn-primary w-100" ${isDisabled} onclick="openNamePetModal(${pet.breed_id}, ${pet.required_points})">${buttonText}</button>
            </div>
          </div>
        </div>
      `;
  ...
  // Open name pet modal
  window.openNamePetModal = function(breedId, cost) {
    currentBreedId = breedId;
    currentBreedCost = cost;
    document.getElementById('petNameInput').value = '';
    
    if (!namePetModal) {
      namePetModal = new bootstrap.Modal(document.getElementById('namePetModal'));
    }
    namePetModal.show();
  };

  // name pet form submission
  document.getElementById('namePetForm').addEventListener('submit', function(e) {
    e.preventDefault();
    
    const petName = document.getElementById('petNameInput').value.trim();
    
    if (!petName) {
      showToast('Invalid Name', 'Please enter a name for your pet', 'warning');
      return;
    }
    ...
    const callback = (responseStatus, responseData) => {
      if (responseStatus === 201 || responseStatus === 200) {
        namePetModal.hide();
        showToast('Welcome Home!', `Hooray! Welcome ${petName} to your family!`, 'success');
    ...
    fetchMethod(currentUrl + `/api/users/${userId}/adopt/pets/${currentBreedId}`, callback, "POST", data, token);
  });
    ...
    const userCallback = (responseStatus, responseData) => {
      ...
      const petCallback = (petStatus, petData) => {
        ...
        if (petData.pet_level < requiredLevel) {
          showToast(
            'Insufficient level!', 
            `${petData.pet_name} needs to be level ${requiredLevel} to unlock this power. Current level: ${petData.pet_level}`,
            'warning'
          );
          return;
        }
        ...
        const unlockCallback = (unlockStatus, unlockData) => {
          if (unlockStatus === 200 || unlockStatus === 201) {
            showToast('Ability Unlocked!', `Hooray! ${petData.pet_name} unlocked a new power!`, 'success');
